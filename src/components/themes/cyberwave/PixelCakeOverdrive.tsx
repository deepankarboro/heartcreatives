  ...
  return (
    <div className="relative min-h-[85vh] flex flex-col items-center justify-center px-4 py-8 max-w-xl mx-auto text-center">
      ...
      <div className="relative my-4 flex flex-col items-center select-none">
        ...
        <div className="relative z-10 w-44 h-16 bg-neutral-900 border-2 border-pink-500 shadow-[0_0_20px_rgba(255,0,127,0.5)] flex items-center justify-center">
          <span className="font-arcade text-xs text-yellow-300">
            {isOvercharged ? 'CRIT 9999' : 'LVL UP!'}
          </span>
        </div>

        <div className="relative z-0 w-64 h-24 bg-neutral-950 border-2 border-cyan-400 shadow-[0_0_25px_rgba(0,240,255,0.4)] flex flex-col items-center justify-center p-2">
          <div className="font-cyber text-xs font-bold text-pink-400">
            {card.recipientName.toUpperCase()} // LEGENDARY
          </div>
          <div className="font-mono text-[10px] text-cyan-200/70 mt-1">
            REACTOR STATUS: {isOvercharged ? 'SUPERCHARGED' : `${power}% CHARGE`}
          </div>
        </div>
